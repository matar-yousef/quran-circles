...
<div class="container-fluid px-4 py-4" style="background-color: #f8fafc; min-height: 100vh;" dir="rtl">
    ...
    <div class="d-flex flex-column flex-md-row justify-content-between align-items-center gap-3 mb-4 p-4 bg-white rounded-4 border border-1 border-light shadow-xs">
        <div class="d-flex align-items-center gap-3">
            ...
            <div>
                <h3 class="fw-bold text-dark mb-1 font-cairo" style="font-size: 1.25rem;">تسجيل اختبار قرآني جديد</h3>
                <p class="text-secondary mb-0" style="font-size: 0.85rem;">إضافة اختبار جديد للطالب وتحديد الدرجة والأجزاء المختبرة</p>
            </div>
        </div>
        <div>
            ...
            <div class="row g-4">
                ...
                <div class="col-md-6">
                    ...
                    <select name="exam_type" class="form-select rounded-pill py-2.5 px-3 text-sm border-light bg-light @error('exam_type') is-invalid @enderror" required>
                        ...
                        <option value="single" {{ old('exam_type') == 'single' ? 'selected' : '' }}>منفرد (جزء واحد)</option>
                        <option value="collective" {{ old('exam_type') == 'collective' ? 'selected' : '' }}>مجتمع (3 أجزاء فأكثر)</option>
                    </select>
                    @error('exam_type')
                    <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                ...
                <div class="col-12">
                    ...
                    <input type="text" name="parts_number" value="{{ old('parts_number') }}" class="form-control rounded-pill py-2.5 px-3 text-sm border-light bg-light @error('parts_number') is-invalid @enderror" placeholder="مثال: الجزء الأول أو الأجزاء (1، 2، 3)" required>
                    @error('parts_number')
                    <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="col-md-6">
                    ...
                    <input type="number" step="0.01" name="grade" value="{{ old('grade') }}" class="form-control rounded-pill py-2.5 px-3 text-sm border-light bg-light @error('grade') is-invalid @enderror" min="0" max="100" placeholder="مثال: 98" required>
                    @error('grade')
                    <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="col-md-6">
                    ...
                    <input type="date" name="exam_date" value="{{ old('exam_date', date('Y-m-d')) }}" class="form-control rounded-pill py-2.5 px-3 text-sm border-light bg-light @error('exam_date') is-invalid @enderror" required>
                    @error('exam_date')
                    <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="col-12">
                    ...
                    <textarea name="notes" rows="3" class="form-control rounded-4 p-3 text-sm border-light bg-light @error('notes') is-invalid @enderror" placeholder="أضف أي ملاحظات تود تسجيلها حول أداء الطالب...">{{ old('notes') }}</textarea>
                    @error('notes')
                    <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
